...
const CalendarDemo: Page = () => {
                    ...
                    if (view === 'edit') {
                        API.put('booking/' + data._id, data).then(function (response) {
                            if (toast && toast.current) {
                                toast.current.show({ severity: 'info', summary: 'Status', detail: 'Booking Created Successfully!!!', life: 3000 });
                            }
                            memberform.resetForm();
                            setShowDialog(false);
                            getbookingData();
                            router.push("/apps/booking");
                        ...
                        API.post('booking', data).then(function (response) {
                            if (toast && toast.current) {
                                toast.current.show({ severity: 'info', summary: 'Status', detail: 'Booking Created Successfully!!!', life: 3000 });
                            }
                            memberform.resetForm();
    ...
    const onEventClick = (e: EventClickArg) => {
        ...
        setShowDialog(true);
        memberform.resetForm();
        memberform.setValues((prevChangeState: any) => ({
            ...prevChangeState,
            ...plainEvent,
    ...
    const onEditClickNew = async (id: any) => {
        try {
            ...
            const memberResponse = await API.get('Members/' + response.data.member._id);
            ...
            // Update the view and show the dialog
            setView("edit");
            setShowDialog(true);
        } catch (error) {
    ...
    const changepackage = async (e: any) => {
        if (e) {
            memberform.setFieldValue('pax', 1);
        }
    };
    ...
    const eventContent = (eventInfo: any) => {
        ...
        const branch = event.extendedProps?.branch?.branch_name;
    ...
    const handleDateChange = (e: any) => {
        ...
        if (startTime) {
            ...
            // Adjust the end time to be one hour after the selected start time
            const endTime = new Date(startTime);
            endTime.setHours(endTime.getHours() + 1);

            // Ensure the end time does not exceed the maxDate
            if (endTime > maxDate) {
                            ...
                            {view === "display" ? (
                                ...
                                <React.Fragment>
                                    <div className="grid">
                                        <div className="col-6">
                                            ...
                                            <p className="flex align-items-center m-0">
                                                ...
                                                <span>
                                                    {formatDateTime(memberform.values.start)}
                                                </span>
                                            </p>
                                        </div>
                                        <div className="col-6">
                                            ...
                                            <p className="flex align-items-center m-0">
                                                <i className="pi pi-fw pi-clock text-700 mr-2"></i>
                                                <span>
                                                    {formatDateTime(memberform.values.end)}
                                                </span>
                                            </p>
                                        </div>
                                        <div className="col-6">
                                        ...
                                        <div className="col-6">
                                            ...
                                            <p className="flex align-items-center m-0">
                                                <i className="pi pi-fw pi-clock text-700 mr-2"></i>
                                                <span>
                                                    {memberform.values.package_name}
                                                </span>
                                            </p>
                                        </div>
                                        <div className="col-6">
                                        ...
                                        <div className="col-6">
                                            ...
                                            <p className="flex align-items-center m-0">
                                                <span>
                                                    {memberform.values.member && memberform.values.member.mobileNumber}
                                                </span>
                                            </p>
                                        </div>
                                        <div className="col-6">
                                            ...
                                            <p className="flex align-items-center m-0">
                                                <span>
                                                    {memberform.values.branch && memberform.values.branch.branch_name}
                                                </span>
                                            </p>
                                        </div>
                                    ...
                                    <Button
                                        ...
                                        label="Edit"
                                        icon="pi pi-pencil"
                                        onClick={() => onEditClickNew(memberform.values._id)}
                                    />
                                </React.Fragment>
                            ) : (
                                <form onSubmit={memberform.handleSubmit} className="field mb-12 col-12">
                                    ...
                                    <div className="grid p-fluid formgrid">
                                        ...
                                        <div className="field mb-6 col-12">
                                            <span className="p-float-label">
                                                <InputNumber id="name" name="name" value={memberform.values.pax}
                                                    onValueChange={(e) => { memberform.setFieldValue('pax', e.value); }}
                                                    min={1} max={balance} />

                                                <label htmlFor="name">Pax</label>
                                            </span>
                                            ...
                                            <div className="col-12 md:col-6 field">
                                                <span className="p-float-label">
                                                    <InputNumber
                                                        ...
                                                        onValueChange={(e) => memberform.setFieldValue('femalecount', e.value)}
                                                        min={-1}
