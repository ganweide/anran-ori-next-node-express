...
function BannerEdit({ params }: { params: { id: string } }) {
    ...
    useEffect(() => {
        ...
        API.get('banner/' + id).then(function (response: any) {
            formik.setFieldValue('image_url', response.data.image_url, false);
            formik.setFieldValue('image_data_url', response.data.image_data_url, false);
        ...
        onSubmit: (data: any) => {
            confirmDialog({
                ...
                accept() {
                    ...
                    API.put('banner/' + id, data).then(function (response) {
                        if (toast && toast.current) {
